...
const dynamicModelWithDBConnection = (dbName, collectionName) => {
  ...
  let connection = mongoose_1.default.createConnection(url, {
    maxPoolSize: 10,
  });
  connection.once("open", () => {
    console.log(
      `Mongodb (${dbName}) called the (${collectionName}) collection!`,
    );
  });
  if (!(collectionName in dynamicModels)) {
    dynamicModels[collectionName] = connection.model(
      collectionName,
      dynamicSchema,
      collectionName,
    );
